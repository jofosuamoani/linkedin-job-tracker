...
SANITIZED="$(sanitize_path "$REPO_ROOT")"
...
# Collect candidate project dirs: the one whose sanitized name matches REPO_ROOT,
...
if [ ${#matching[@]} -eq 0 ]; then
    echo "No Claude sessions found for $REPO_ROOT (checked ${#candidates[@]} candidate dir(s))"
    exit 1
fi

mkdir -p "$OUTPUT_DIR"

echo "Packaging Claude sessions for $REPO_ROOT (${#matching[@]} project dir(s))..."
rels=()
