...
for base in "${editor_bases[@]}"; do
    ...
    for task_dir in "$tasks_dir"/*/; do
        ...
        # Look for "Current Working Directory (REPO_ROOT)" or a subpath under it.
...
if [ "$matched" -eq 0 ]; then
    echo "No Cline sessions found for $REPO_ROOT"
    exit 0
fi

mkdir -p "$OUTPUT_DIR"

echo "Packaging $matched Cline task(s) for $REPO_ROOT..."
